...
local function transformAmmoDeploy(api) --在切换形态时重新分配弹药量和热量
    ...
    local max_ammo = api:getMaxAmmoCount() --枪管里也有一发
    ...
    if (api:getMaxAmmoCount()<AmmoFromHeat) then --由于算上枪管，所以可能多一发，那样就先照顾枪管
        AmmoFromHeat = api:getMaxAmmoCount()
